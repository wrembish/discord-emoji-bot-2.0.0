...
with open('conversionmap.txt') as f:
...
with open('guildids.txt') as f:
  guild_ids = simplejson.load(f)

with open('built_in_messages.txt') as f:
  built_in_messages = json.load(f)
...
async def _pog(ctx:SlashContext):
  await ctx.send(built_in_messages['pog'])
...
async def _dude(ctx:SlashContext):
  await ctx.send(built_in_messages['dude'])
...
async def _cuss(ctx:SlashContext):
  await ctx.send(built_in_messages['cuss'])
...
async def _ez(ctx:SlashContext):
  await ctx.send(built_in_messages['ez'])
...
async def _matt2(ctx:SlashContext):
  await ctx.send(built_in_messages['star'])
...
async def _julius(ctx:SlashContext):
  await ctx.send(built_in_messages['julius'])
  ...
  if content.startswith('!emoji '):
    ...
    if content == "allow slash commands":
      ...
      if message.guild.id in guild_ids:
        ...
        with open('guildids.txt','w') as f:
          simplejson.dump(guild_ids,f)
